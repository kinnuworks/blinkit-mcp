 ...
 */
export function extractProducts(root) {
    const out = new Map();
    const seen = new Set();
    const visit = (node) => {
        ...
        const obj = node;
        const cartItem = obj?.atc_action?.add_to_cart?.cart_item ??
        ...
        const looksProduct = pid !== undefined && (cartItem || obj.atc_action || obj.normal_price || obj.mrp || obj.assets);
...
export function parseReorderIds(deeplink) {
    if (!deeplink)
        return [];
...
export function extractOrders(root) {
    const orders = [];
    const seen = new Set();
    const visit = (node) => {
