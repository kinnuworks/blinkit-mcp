...
export async function request(path, opts = {}) {
    ...
    if (s.auth_key)
        headers.auth_key = s.auth_key;
    if (s.lat !== undefined)
        headers.lat = String(s.lat);
    if (s.lon !== undefined)
        headers.lon = String(s.lon);
    ...
    if (wantAuth && s.access_token)
        headers.access_token = s.access_token;
    ...
    if (wantAuth && s.access_token)
        cookies.push(`gr_1_accessToken=${encodeURIComponent(s.access_token)}`);
    headers.cookie = cookies.join("; ");
    let body;
    if (opts.form) {
        headers["content-type"] = "application/x-www-form-urlencoded";
        body = new URLSearchParams(Object.fromEntries(Object.entries(opts.form).map(([k, v]) => [k, String(v)]))).toString();
    }
 ...
 */
export async function ensureAuthKey() {
    const s = await loadSession();
    if (s.auth_key)
        return s.auth_key;
    const res = await request("/v2/accounts/auth_key/", { headers: { req_key: REQ_KEY } });
    if (!res?.auth_key)
        throw new BlinkitError("Failed to obtain auth_key", 0, res);
    await updateSession({ auth_key: res.auth_key });
    return res.auth_key;
}
export function requireLogin(s) {
    if (!s.access_token) {
        throw new BlinkitError("Not logged in. Call send_otp then verify_otp first.", 401);
    }
}
export function requireLocation(s) {
